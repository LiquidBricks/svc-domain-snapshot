...
import { updateSnapshotState } from '../../../../../../core/domain/vertex/stateMachine/started/handler.js'
import { DiagnosticError, makeDiagnostics } from '../../../../../helpers.mjs'

function args({ componentStateIds = ['component-state-1'], currentState, calls = [] } = {}) {
  return {
    rootCtx: {
      dataMapper: {
        query: {
...
test('preserves the component snapshot while updating instance lifecycle state', async () => {
  const calls = []
  const currentState = {
  ...
  assert.deepEqual(result.delta, { 'instance.state': 'running' })
  ...
  test(`rejects an invalid linked snapshot count of ${componentStateIds.length}`, async () => {
    await assert.rejects(
      updateSnapshotState(args({
        componentStateIds,
        currentState: { 'instance.state': 'created' },
      })),
      DiagnosticError,
    )
  })
